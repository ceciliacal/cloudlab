...
for i in range(1, params.edgeNodes + 1):
    ...
    node.addService(rspec.Install(
        url="https://raw.githubusercontent.com/ceciliacal/cloudlab/main/startup.tar.gz",
        path="/usr/local"))
...
for i in range(1, params.clientNodes + 1):
    ...
    node.addService(rspec.Install(url="https://raw.githubusercontent.com/ceciliacal/cloudlab/main/startup.tar.gz", path="/usr/local"))
    node.addService(rspec.Execute(shell="bash", command="bash /usr/local/startup.sh"))

    #node.addService(rspec.Execute(shell="bash", command="wget -O -  https://raw.githubusercontent.com/ceciliacal/cloudlab/main/startup.sh | bash"))
